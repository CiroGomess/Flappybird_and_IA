...
def desenhar_tela(tela, passaros, canos, chao, pontos):
    ...
    # Verificando se a IA está jogando para adicionar as gerações
    if ai_jogando:
...
def main(genomas, config):
    ...
    # Verificando se a ia está jogando
    if ai_jogando:
        redes = []
        lista_genomas = []
        passaros = []
        ...
        for _, genoma in genomas:
            ...
            # adicionando rede neural na lista de redes
            redes.append(rede)
    ...
    while rodando:
        ...
        # Descobrindo qual cano olhar - IA
        indice_cano = 0
        if len(passaros) > 0:
        ...
        for i, passaro in enumerate(passaros):
            passaro.mover()
            # aumetar um pouco a fitness do passaro
            lista_genomas[i].fitness += 0.1
            output = redes[i].activate((passaro.y,
                                        abs(passaro.y - canos[indice_cano].altura),
                                        abs(passaro.y - canos[indice_cano].pos_base)))
            # output > 0.5 passaro pula
            if output[0] > 0.5:
                passaro.pular()
